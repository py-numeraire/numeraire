"""Native evaluators (numpy/scipy, no heavy deps) — the performance family (SPEC §2.2).

Evaluators dispatch by capability and emit rows of the tidy result schema, so the metric
always matches the object (landmine #2: VoC's headline is *timing Sharpe*, not R²). Each
...
class OOSR2Evaluator:
    ...
    for predictive-regression methods (landmine #2) — e.g. 1/A's published dp Table 3.
    """
