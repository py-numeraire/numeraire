...
from numeraire.core.sorts import sort_portfolios
...
def test_two_bin_split_and_equal_weight_returns() -> None:
    ...
    res = sort_portfolios(signal, returns, n_bins=2)
...
def test_value_weighting_uses_weights() -> None:
    ...
    res = sort_portfolios(signal, returns, n_bins=2, weights=weights)
...
def test_nyse_breakpoints_shift_cutoffs() -> None:
    ...
    res = sort_portfolios(signal, returns, n_bins=2, breakpoint_universe=universe)
...
def test_validation_guards() -> None:
    ...
    with pytest.raises(ValueError, match="n_bins must be >= 2"):
        sort_portfolios(s, s, n_bins=1)
    with pytest.raises(ValueError, match="direction must be"):
        sort_portfolios(s, s, n_bins=2, direction=0)
    with pytest.raises(ValueError, match="aligned on the same"):
        sort_portfolios(s, s.rename(columns={"a": "z"}), n_bins=2)
